...
const SITE_PRESETS = {
	...
	'bwin':           { bonus: 'freebet_lose',   min: 50,  maxBonus: 50  },
	'Daznbet':        { bonus: 'freebet_lose',   min: 100, maxBonus: 100 },
	'Feelingbet':     { bonus: 'freebet_lose',   min: 200, maxBonus: 200 },
	'Olybet':         { bonus: 'freebet_lose',   min: 50,  maxBonus: 50  },
	'Parions Sports': { bonus: 'freebet_lose',   min: 100, maxBonus: 100 },
	'PMU':            { bonus: 'freebet_lose',   min: 100, maxBonus: 100 },
	'Pokerstars':     { bonus: 'freebet_lose',   min: 50,  maxBonus: 50  },
	'Unibet':         { bonus: 'freebet_lose',   min: 100, maxBonus: 100 },
	'VBET':           { bonus: 'freebet_lose',   min: 100, maxBonus: 100 },
	'Winamax':        { bonus: 'freebet_always', min: 100, maxBonus: 100 },
};
...
function makeBookie(i) {
	const defaults = [
		{ site: 'Betclic',  name: 'Betclic',  outcome: 'Issue 1', odds: 2,   bonus: 'freebet_lose',   min: 100, maxBonus: 100 },
		{ site: 'Winamax',  name: 'Winamax',  outcome: 'Issue 2', odds: 3.5, bonus: 'freebet_always', min: 100, maxBonus: 100 },
		{ site: 'Unibet',   name: 'Unibet',   outcome: 'Issue 3', odds: 3.5, bonus: 'freebet_lose',   min: 100, maxBonus: 100 },
	];
	return defaults[i] || { site: '-Autre-', name: `Site ${i + 1}`, outcome: `Issue ${i + 1}`, odds: 2.0, bonus: 'freebet_lose', min: 100, maxBonus: 100 };
}
                ...
                <tr>
                  <td>
                    <div class="bookie-num-badge">
                      <div style="width:8px;height:8px;border-radius:50%;background:${ACCENT_COLORS[i]}"></div>
                      ${i + 1}
                    </div>
                  </td>
                  <td>
                  ...
                  <td><input type="text" value="${b.outcome}" oninput="bookies[${i}].outcome=this.value" onclick="this.select()" placeholder="Ex: Équipe gagne" /></td>
                  <td>
                    <div class="numinput">
                      ...
                      <div class="nbtn-wrap">
                        <button class="nbtn" onclick="incrementOdds(${i})" type="button">▲</button>
                        <button class="nbtn" onclick="decrementOdds(${i})" type="button">▼</button>
                      </div>
                    </div>
                  </td>
                  <td>
...
function incrementOdds(i) {
	...
	const newVal = Math.round(((parseFloat(input.value) || 1.01) + 0.01) * 100) / 100;
	input.value = newVal; bookies[i].odds = newVal;
}
function decrementOdds(i) {
	const input = document.getElementById(`odds-${i}`);
	const newVal = Math.max(1.01, Math.round(((parseFloat(input.value) || 1.01) - 0.01) * 100) / 100);
	input.value = newVal; bookies[i].odds = newVal;
}
...
function calculate() {
	...
	rows.forEach((row, i) => {
		...
		// inputs[0] = site select, inputs[1] = name, inputs[2] = outcome,
		// inputs[3] = odds, inputs[4] = bonus select, inputs[5] = min, inputs[6] = max
		bookies[i].name = inputs[1].value;
		bookies[i].outcome = inputs[2].value;
		bookies[i].odds = parseFloat(inputs[3].value) || 1.01;
		bookies[i].bonus = inputs[4].value;
		bookies[i].min = parseFloat(inputs[5].value) || 0;
		bookies[i].maxBonus = parseFloat(inputs[6].value) || 0;
	});
	...
	for (let iter = 0; iter < 20; iter++) {
		...
		for (let k = 1; k < n; k++) {
			const needed = (eff[k] * (bookies[k].min || 0) - C[0] + C[k]) / eff[0];
			if (needed > s0) s0 = needed;
		}
		if (s0 < (bookies[0].min || 0)) s0 = bookies[0].min || 0;

		const candidate = bookies.map((b, j) => j === 0 ? s0 : (eff[0] * s0 + C[0] - C[j]) / eff[j]);
...
function showResults({ stakes, gains, avgGain, totalStaked, roi, convRate, effectiveConv, capped, bonusAmount }) {
	...
	document.getElementById('stakes-grid').innerHTML = bookies.map((b, i) => {
		...
		return `
          <div class="stake-card" style="border-left-color:${ACCENT_COLORS[i]}">
            <div class="stake-bookie" style="color:${ACCENT_COLORS[i]}">${b.name}</div>
